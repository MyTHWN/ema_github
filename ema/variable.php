        ...
        if (is_array($this->variable) && array_key_exists("answer", $this->variable)) {
            ...
            if ($this->array == false) {
                ...
                if ($this->answertype == ANSWER_TYPE_SETOFENUMERATED || $this->answertype == ANSWER_TYPE_MULTIDROPDOWN) {
                    ...
                    $matches = array();
                    preg_match("/(_[0-9]+_){1}/", $this->completevariablename, $matches);
                    if (sizeof($matches) > 0) {
                        ...
                        global $engine;
                        $values = explode(SEPARATOR_SETOFENUMERATED, $engine->getAnswer(preg_replace("/(_[0-9]+_){1}/", "", $this->completevariablename)));
            ...
            // an array
                ...
                // specific array instance, e.g. Q1[1,1,1]
                    ...
                    if ($this->answertype == ANSWER_TYPE_SETOFENUMERATED || $this->answertype == ANSWER_TYPE_MULTIDROPDOWN) {
                        ...
                        // specific set of enumerated option
                        $matches = array();
                        preg_match("/(_[0-9]+_)/", $this->completevariablename, $matches);
                        if (sizeof($matches) > 0) {
                            ...
                            // get values of set of enum question as a whole
                            global $engine;
                            $values = explode(SEPARATOR_SETOFENUMERATED, $engine->getAnswer(preg_replace("/(_[0-9]+_)/", "", $this->completevariablename)));
        ...
        if ($this->array == false) {
            ...
            if ($this->answertype == ANSWER_TYPE_SETOFENUMERATED || $this->answertype == ANSWER_TYPE_MULTIDROPDOWN) {
                ...
                if (sizeof($matches) > 0) {
                    ...
                    // get current values
                    global $engine;
                    $real = preg_replace("/(_[0-9]+_){1}/", "", $this->completevariablename);
...
//echo $this->completevariablename . "[" . $key . "]" . '---' . $value;
            ...
            // specific instance
                ...
                if ($this->answertype == ANSWER_TYPE_SETOFENUMERATED || $this->answertype == ANSWER_TYPE_MULTIDROPDOWN) {
                    ...
                    $matches = array();
                    preg_match("/(_[0-9]+_){1}/", $varname, $matches);
                    if (sizeof($matches) > 0) {
                        ...
                        // get current values
                        global $engine;
                        $real = preg_replace("/(_[0-9]+_){1}/", "", $varname);
                        $values = explode(SEPARATOR_SETOFENUMERATED, $engine->getAnswer($real));
