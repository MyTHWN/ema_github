...
class Compiler {
    ...
    function addSetFill($function, &$node, $instruction) {
        ...
        if (sizeof($this->loops) == 0 && sizeof($this->whiles) == 0) {
            ...
            // check for end of for loop/ while loop
            $enddo = -1;
    ...
    function generateGetFills($variables = array()) {
        ...
        foreach ($variables as $var) {
            ...
            if (inArray($t, array(ANSWER_TYPE_STRING, ANSWER_TYPE_OPEN, ANSWER_TYPE_RANGE, ANSWER_TYPE_DOUBLE, ANSWER_TYPE_INTEGER, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_DROPDOWN, ANSWER_TYPE_DATE, ANSWER_TYPE_DATETIME, ANSWER_TYPE_TIME, ANSWER_TYPE_SLIDER))) {
...
//echo  $rule . "<hr>";
        ...
        $find = array();
        $locate = '/' . PATTERN_ALPHANUMERIC . PATTERN_CASE_INSENSITIVE . LOGICAL_IN . PATTERN_ALPHANUMERIC . '/i';
        //echo $locate . "<hr>";
        if (preg_match_all($locate, $rule, $find, PREG_SET_ORDER)) {
            foreach ($find as $found) {
                //print_r($found);
...
//echo $rule;
    ...
    function findEndIf($rgid) {
        ...
        for ($cnt = ($rgid + 1); $cnt <= sizeof($this->instructions); $cnt++) {
            ...
            } else if (startsWith($rule, ROUTING_IF)) {
    ...
    function findNextAfterIf($rgid, $iftype) {
        ...
        for ($cnt = ($rgid + 1); $cnt <= sizeof($this->instructions); $cnt++) {
            ...
            if (startsWith($rule, "/*")) {
                ...
                $level++;
            } else if (startsWith($rule, ROUTING_ELSEIF)) {
