                    ...
                    while ($row = $this->db->getRow($res)) {
                        ...
                        for ($i = 0; $i < sizeof($varnames); $i++) {
                            ...
                            if ($variable) {
                                ...
                                $line .= $varnames[$i] . $this->separator . $variable->getLanguage() . $this->separator . $variable->getMode() . $this->separator . $variable->getVersion() . $this->separator . $variable->getTs();
                        ...
                        if ($res2) {
                            ...
                            for ($i = 0; $i < sizeof($varnames); $i++) {
                                ...
                                if (isset($currentrecord[strtoupper($varnames[$i])])) {
                                    ...
                                    $line .= $varnames[$i] . $this->separator . $info["language"] . $this->separator . $info["mode"] . $this->separator . $info["version"] . $this->separator . $info["ts"];
                                    fwrite($this->csvhandle, $line . "\n");
                                    $info = null;
                                    unset($info);
                                    $line = null;
        ...
        if ($res) {
            if ($this->db->getNumberOfRows($res) == 0) {
                ...
                while ($row = $this->db->getRow($res)) {
                    ...
                    if (!(sizeof($languages) == 0 || (sizeof($languages) > 0 && inArray($row["language"], $languages)))) {//
                        continue;
                    } else if (!(sizeof($modes) == 0 || (sizeof($modes) > 0 && inArray($row["mode"], $modes)))) {//
                        continue;
                    }
...
//print_r($row);        
        ...
        $data2 = "";
        $select = "select primkey, sum(timespent) as total, sum(timespent)/60 as total2, avg(timespent) as average, count(*) as cnt, language, mode from " . $this->getProperty(DATA_OUTPUT_MAINDATATABLE) . "_consolidated_times where suid=" . $this->suid . "  and  timespent < $cutoff and length(primkey) >= " . $this->minprimkeylength . " and length(primkey) < " . $this->maxprimkeylength . " group by primkey order by primkey asc";
        $res = $this->db->selectQuery($select);
        if ($this->db->getNumberOfRows($res) > 0) {
            while ($row = $this->db->getRow($res)) {
                //if (is_numeric($row["primkey"])) {
    ...
    function generate() {
        ...
        // TODO: check if we have data names from last time data export was done
        //$survey = new Survey($this->suid);
        $query = "select * from " . $this->getProperty(DATA_OUTPUT_MAINTABLE) . "_dataexports where suid=" . $this->suid . " and length(primkey) >= " . $this->minprimkeylength . " and length(primkey) < " . $this->maxprimkeylength . $extracompleted . " order by ts";
        $res = null; //$this->db->selectQuery($query);
        $datanames = array();
        $lastts = "";
        $datanamestring = "";
        if ($res) {
            if ($this->db->getNumberOfRows($res) > 0) {
                ...
                $datanames = explode("~", gzuncompress($row["datanames"]));
                $lastts = $row["ts"];
            }
        }

// END TODO
        // find any (additional) data names    
        $extra = "";
        if ($lastts != "") {
            $extra = " and ts > '" . $lastts . "'";
        }
        ...
        if ($this->getProperty(DATA_OUTPUT_TYPE) == DATA_OUTPUT_TYPE_DATARECORD_TABLE) {
            $query = "select count(*) as cnt from " . $this->getProperty(DATA_OUTPUT_MAINDATATABLE) . "_datarecords where suid=" . $this->suid . " and length(primkey) >= " . $this->minprimkeylength . " and length(primkey) <= " . $this->maxprimkeylength . $extracompleted;
            $res = $this->db->selectQuery($query);
            $row = $this->db->getRow($res);
        ...
        if ($this->getProperty(DATA_OUTPUT_TYPE) == DATA_OUTPUT_TYPE_DATARECORD_TABLE) {
            ...
            $query = "select primkey, $decrypt from " . $this->getProperty(DATA_OUTPUT_MAINDATATABLE) . "_datarecords where suid=" . $this->suid . " and length(primkey) >= " . $this->minprimkeylength . " and length(primkey) <= " . $this->maxprimkeylength . $extracompleted . " order by primkey";
            $res = $this->db->selectQuery($query);
            ...
            if ($res) {
                if ($this->db->getNumberOfRows($res) == 0) {
                    ...
                    while ($row = $this->db->getRow($res)) {
                        ...
                        $query = "select primkey, variablename, $decrypt, language, mode, version, dirty from " . $this->getProperty(DATA_OUTPUT_MAINDATATABLE) . "_data where suid=" . $this->suid . " and primkey='" . $row["primkey"] . "' order by variablename";
    ...
    function getDataTableValue($prim, $field, $modes = array(), $languages = array(), $versions = array()) {
        ...
        if (isset($this->currentrecord[strtoupper($field)])) {
            ...
            if (!(sizeof($languages) == 0 || (sizeof($languages) > 0 && inArray($arr["language"], $languages)))) {//
                $value = null;
            } else if (!(sizeof($modes) == 0 || (sizeof($modes) > 0 && inArray($arr["mode"], $modes)))) {//
                $value = null;
            }
            // match
            else {
    ...
    function getValue($primkey, $record, $fieldname) {
        ...
        if ($this->getProperty(DATA_OUTPUT_TYPE) == DATA_OUTPUT_TYPE_DATA_TABLE) {
            $value = $this->getDataTableValue($primkey, $fieldname, $modes, $languages, $versions);
        }
        // from _datarecords table
            ...
            if ($variableobject) {
                ...
                /* no match on language, mode and version, then treat as never asked */
                if (!(sizeof($languages) == 0 || (sizeof($languages) > 0 && inArray($variableobject->getLanguage(), $languages)))) {//
                    $value = null;
                } else if (!(sizeof($modes) == 0 || (sizeof($modes) > 0 && inArray($variableobject->getMode(), $modes)))) {//
                    $value = null;
                }
                // match
                else {
