...
class DisplayQuestionBasic extends Display {
    ...
    function showQuestion($variablenames, $rgid, $groupname = "") {
        ...
        /* add post parameters */
        $returnStr = setSessionParamsPost(array(SESSION_PARAM_LASTACTION => $this->engine->getLastSurveyAction(), SESSION_PARAM_SURVEY => $survey->getSuid(), SESSION_PARAM_PRIMKEY => $this->primkey, SESSION_PARAM_RGID => $rgid, SESSION_PARAM_VARIABLES => $realvariables, SESSION_PARAM_GROUP => $groupname, SESSION_PARAM_LANGUAGE => getSurveyLanguage(), SESSION_PARAM_MODE => getSurveyMode(), SESSION_PARAM_VERSION => getSurveyVersion(), SESSION_PARAM_TIMESTAMP => time(), SESSION_PARAM_SEID => $this->engine->getSeid(), SESSION_PARAM_MAINSEID => $this->engine->getMainSeid()));
        ...
        if ($queryobject) {
            ...
            if ($this->engine->getForward() == true) {
                if ($queryobject->getShowBackButton() == BUTTON_YES) {
                    $back = true;
                    $returnStrButtons .= '<button disabled="disabled" class="btn btn-default" name="backbutton" id="uscic-backbutton" value="' . $this->engine->replaceFills($queryobject->getLabelBackButton()) . '" type="button" onclick=\'disableButtons(); ' . $screenshotscript . 'document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelBackButton())) . '"; ' . $unmask . ' document.getElementById("form").submit();\'>' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelBackButton()), $buttonformat) . '</button>';
                }
            } else {
                ...
                if (trim($groupname) == "") {
                    if ($doempty || $doerror) {
                        ...
                        if ($doempty) {
                            ...
                            //if ($ifempty == IF_EMPTY_WARN) {
                            //   $extra_empty_check .= "else if (document.getElementById(\"em\").value == 1) {clearForm();}";
                            //}
                        }
                        ...
                        if ($doerror) {
                            ...
                            //if ($iferror == IF_ERROR_WARN) {
                            //    $extra_error_check .= "else if (document.getElementById(\"er\").value == 1) {clearFormError();}";
                            //}
                        }
                        if ($lastcheck != "") {
                            $lastcheck = "if (" . $lastcheck . ") { enableButtons(); return false;}";
                        }
                        //$returnStrButtons .= '<button disabled="disabled" class="btn btn-default" id="uscic-nextbutton" type="button" onclick=\'disableButtons(); ' . $screenshotscript . ' clearForm();' . $extra_empty_check . ' ' . $extra_error_check . ' ' . $lastcheck . ' document.getElementById("navigation").value="' . addslashes($queryobject->getLabelNextButton()) . '"; ' . $unmask . ' document.getElementById("form").submit();\' value="' . $queryobject->getLabelNextButton() . '">' . $this->applyFormatting($queryobject->getLabelNextButton(), $buttonformat) . '</button>';
                        $returnStrButtons .= '<button disabled="disabled" class="btn btn-default" name="nextbutton" id="uscic-nextbutton" type="button" onclick=\'disableButtons(); ' . $screenshotscript . ' clearForm();' . $extra_empty_check . ' ' . $extra_error_check . ' ' . $lastcheck . ' document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelNextButton())) . '"; ' . $unmask . ' $("#form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelNextButton()) . '">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelNextButton()), $buttonformat) . '</button>';
                    }
                    //  no empty and/or error checking
                    else {
                        $returnStrButtons .= '<button disabled="disabled" class="btn btn-default" name="nextbutton" id="uscic-nextbutton" type="button" onclick=\'disableButtons(); ' . $screenshotscript . 'document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelNextButton())) . '"; ' . $unmask . ' document.getElementById("form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelNextButton()) . '">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelNextButton()), $buttonformat) . '</button>';
                    }
                }
                //  group
                else {
                    ...
                    if ($doempty || $doerror) {
                        ...
                        if ($doempty) {
                            ...
                            $extra_empty_check .= "empty = true;}";
                            //if ($ifempty == IF_EMPTY_WARN) {
                            //    $extra_empty_check .= "else if (document.getElementById(\"em\").value == 1) {}";
                            // }
                        }
                        ...
                        if ($doerror) {
                            ...
                            $extra_error_check .= "error = true;}";
                            //if ($iferror == IF_ERROR_WARN && $ifempty != IF_EMPTY_NOTALLOW) {
//                                $extra_error_check .= "else if (document.getElementById(\"er\").value == 1) {clearForm();}";
                            //                          }
                        }
                        if ($lastcheck != "") {
                            $lastcheck = "if (" . $lastcheck . ") { enableButtons(); return false;}";
                        }
                        $returnStrButtons .= '<button class="btn btn-default" name="nextbutton" id="uscic-nextbutton" type="button" onclick=\'disableButtons(); ' . $screenshotscript . 'clearForm();' . $extra_empty_check . ' ' . $extra_error_check . ' ' . $lastcheck . ' document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelNextButton())) . '"; ' . $unmask . ' document.getElementById("form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelNextButton()) . '">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelNextButton()), $buttonformat) . '</button>';
                    }
                    //  no empty and/or error checking
                    else {
                        $returnStrButtons .= '<button class="btn btn-default" name="nextbutton" id="uscic-nextbutton" type="button" onclick=\'disableButtons(); ' . $screenshotscript . 'document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelNextButton())) . '"; ' . $unmask . ' document.getElementById("form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelNextButton()) . '">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelNextButton()), $buttonformat) . '</button>';
                    }
                }
            }
                ...
                if ($this->dkrfna == false) {
                    if ($queryobject->getShowDKButton() == BUTTON_YES || $queryobject->getShowRFButton() == BUTTON_YES || $queryobject->getShowNAButton() == BUTTON_YES || $queryobject->getShowRemarkButton() == BUTTON_YES) {
                        ...
                        if ($queryobject->getShowDKButton() == BUTTON_YES) {
                            ...
                            $returnStrButtons .= '<button disabled="disabled" name="dkbutton" class="btn btn-default ' . $highlight . '" id="uscic-dkbutton" onclick=\'disableButtons(); ' . $screenshotscript . 'document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelDKButton())) . '"; document.getElementById("form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelDKButton()) . '" type="button">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelDKButton()), $buttonformat) . '</button>';
                        }
                        if ($queryobject->getShowRFButton() == BUTTON_YES) {
                            ...
                            $returnStrButtons .= '<button disabled="disabled" name="rfbutton" class="btn btn-default ' . $highlight . '" id="uscic-rfbutton" onclick=\'disableButtons(); ' . $screenshotscript . 'document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelRFButton())) . '"; document.getElementById("form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelRFButton()) . '" type="button">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelRFButton()), $buttonformat) . '</button>';
                        }
                        if ($queryobject->getShowNAButton() == BUTTON_YES) {
                            ...
                            $returnStrButtons .= '<button disabled="disabled" name="nabutton" class="btn btn-default ' . $highlight . '" id="uscic-nabutton" onclick=\'disableButtons(); ' . $screenshotscript . 'document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelNAButton())) . '"; document.getElementById("form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelNAButton()) . '" type="button">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelNAButton()), $buttonformat) . '</button>';
                        }
                    }
                }
            }
            ...
            if ($queryobject->getShowUpdateButton() == BUTTON_YES) {
                ...
                $returnStrButtons .= '<button disabled="disabled" class="btn btn-default" name="updatebutton" id="uscic-updatebutton" onclick=\'disableButtons(); ' . $screenshotscript . 'document.getElementById("navigation").value="' . addslashes($this->engine->replaceFills($queryobject->getLabelUpdateButton())) . '"; document.getElementById("form").submit();\' value="' . $this->engine->replaceFills($queryobject->getLabelUpdateButton()) . '" type="button">' . $this->applyFormatting($this->engine->replaceFills($queryobject->getLabelUpdateButton()), $buttonformat) . '</button>';
            }
        ...
        // no object that holds display settings --> THIS SHOULD NEVER HAPPEN
            ...
            /*if ($queryobject->getShowRemarkButton() == BUTTON_YES) {
                ...
                if ($this->dkrfna == true && $queryobject->getShowRemarkButton() != BUTTON_YES) {
                    $returnStrButtons .= '<span class="pull-right">';
                }
                $rem = $this->engine->loadRemark();
                $returnStrButtons .= $this->showRemarkModal($queryobject, $qa, $rem);
                ...
                $returnStrButtons .= '<button disabled="disabled" class="btn btn-default ' . $highlight . '" id="uscic-remarkbutton" data-toggle="modal" data-target="#remarkmodal" type="button">' . $this->applyFormatting($queryobject->getLabelRemarkButton(), $buttonformat) . '</button>';
            }*/
            if (($this->showdk == true && $this->dkrfna == false) || $queryobject->getShowUpdateButton() == BUTTON_YES || $queryobject->getShowRemarkButton() == BUTTON_YES) {
                $returnStrButtons .= '</span>';
            }
    ...
    function getErrorTextString($name) {
        ...
        foreach ($messages as $k => $v) {
            $str .= $k . "='" . convertHTLMEntities($v, ENT_QUOTES) . "' ";
        }
        return $str;
    }
            ...
            case ANSWER_TYPE_OPEN: //open
                ...
                if ($inline) {
                    ...
                    $returnStr .= '<div class="form-group uscic-formgroup' . $inlineclass . '">
                                ...
                                <textarea  style="width:330px" ' . $placeholder . $inlinestyle . ' ' . $inlinejavascript . ' ' . $qa . ' ' . $this->getErrorTextString($varname) . ' id=' . $id . ' class="form-control uscic-form-control ' . $dkrfnaclass . '" name=' . $varname . '>' . convertHTLMEntities($previousdata, ENT_QUOTES) . '</textarea>
                                    </div>' . $dkrfna . '
                                </label>                                
                                </div>';
                }
  ...
  <ul class="dropdown-menu" role="menu">';
        ...
        foreach ($langs as $key => $lang) {
            if (inArray($lang["value"], $allowed)) {
                ...
                $returnStr .= '<li><a href=# onclick=\'document.getElementById("r").value="' . setSessionsParamString(array_merge(array(SESSION_PARAM_LASTACTION => $this->engine->getLastSurveyAction(), SESSION_PARAM_SURVEY => $survey->getSuid(), SESSION_PARAM_PRIMKEY => $this->primkey, SESSION_PARAM_RGID => $rgid, SESSION_PARAM_VARIABLES => $variablenames, SESSION_PARAM_GROUP => $template, SESSION_PARAM_MODE => getSurveyMode(), SESSION_PARAM_VERSION => getSurveyVersion(), SESSION_PARAM_LANGUAGE => $current, SESSION_PARAM_TIMESTAMP => time(), SESSION_PARAM_SEID => $this->engine->getSeid(), SESSION_PARAM_MAINSEID => $this->engine->getMainSeid()), array(SESSION_PARAM_NEWLANGUAGE => $lang["value"]))) . '"; document.getElementById("navigation").value="' . NAVIGATION_LANGUAGE_CHANGE . '"; $("#form").submit(); \'>' . $lang["name"] . $check . '</a></li>';
            }
        }
        $returnStr .= '
  </ul>
