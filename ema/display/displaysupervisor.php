...
//		$query = 'select DATE(ts) as dateobs, count(*) as cntobs, primkey from ' . dbConfig::dbSurvey() . '_contacts where length(primkey) > 8  group by DATE(ts) order by ts asc'; 
        if ($rorh == 1) { //houseohld level
            ...
            $query .= 'where ' . $userStr . getTextmodeStr() . ' t1.ts > "' . date('Y-m-d', config::graphStartDate()) . ' 23:59:99" AND  t2.primkey is not null group by DATE(t1.ts) order by t1.ts asc';
        } else {
            $query = 'select DATE(t1.ts) as dateobs, count(*) as cntobs, t1.primkey from ' . dbConfig::dbSurvey() . '_contacts as t1 ';
    ...
    function getFieldNotNull($rorh = 1, $fieldname) {
        ...
        if ($rorh == 1) { //houseohld level
            $query = 'select DATE(t1.ts) as dateobs, count(*) as cntobs, t1.primkey from ' . dbConfig::dbSurveyData() . '_data as t1 ';
            $query .= 'left join ' . dbConfig::dbSurvey() . '_households t2 on t2.primkey = t1.primkey ';
            $query .= 'where ' . $userStr . getTextmodeStr('t1.') . ' t1.ts > "' . date('Y-m-d', config::graphStartDate()) . ' 23:59:99" AND suid = ' . $rorh . ' and variablename="' . $fieldname . '" and answer is not null group by DATE(t1.ts) order by t1.ts asc';
        } else {
            $query = 'select DATE(t1.ts) as dateobs, count(*) as cntobs, t1.primkey from ' . dbConfig::dbSurveyData() . '_data as t1 ';
            $query .= 'left join ' . dbConfig::dbSurvey() . '_respondents t2 on t2.primkey = t1.primkey ';
            $query .= 'where ' . $userStr . getTextmodeStr('t1.') . ' t1.ts > "' . date('Y-m-d', config::graphStartDate()) . ' 23:59:99" AND suid = ' . $rorh . ' and variablename="' . $fieldname . '" and answer is not null group by DATE(t1.ts) order by t1.ts asc';
    ...
    function getFieldIsNull($rorh = 1, $fieldname) {
        ...
        if ($rorh == 1) { //houseohld level
            $query = 'select DATE(t1.ts) as dateobs, count(*) as cntobs, t1.primkey from ' . dbConfig::dbSurveyData() . '_data as t1 ';
            $query .= 'left join ' . dbConfig::dbSurvey() . '_households t2 on t2.primkey = t1.primkey ';
            $query .= 'where ' . $userStr . getTextmodeStr('t1.') . ' t1.ts > "' . date('Y-m-d', config::graphStartDate()) . ' 23:59:99" AND suid = ' . $rorh . ' and variablename="' . $fieldname . '" and answer is null group by DATE(t1.ts) order by t1.ts asc';
        } else {
            $query = 'select DATE(t1.ts) as dateobs, count(*) as cntobs, t1.primkey from ' . dbConfig::dbSurveyData() . '_data as t1 ';
            $query .= 'left join ' . dbConfig::dbSurvey() . '_respondents t2 on t2.primkey = t1.primkey ';
            $query .= 'where ' . $userStr . getTextmodeStr('t1.') . ' t1.ts > "' . date('Y-m-d', config::graphStartDate()) . ' 23:59:99" AND suid = ' . $rorh . ' and variablename="' . $fieldname . '" and answer is null group by DATE(t1.ts) order by t1.ts asc';
        }
        //$query = 'select DATE(ts) as dateobs, count(*) as cntobs, primkey from recruit_data ';
        //$query .= 'where ' . getTextmodeStr('') . ' ts > "' . date('Y-m-d', config::graphStartDate()) . ' 23:59:99" AND suid = ' . $rorh . ' and variablename="' . $fieldname . '" and answer is null group by DATE(ts) order by ts asc'; 
        //echo '<br/><br/><br/>' . $query;                
