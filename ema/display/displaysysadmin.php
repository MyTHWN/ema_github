...
';
    ...
    function showSections($sections) {
        ...
        if (sizeof($sections) > 0) {
            ...
            $returnStr .= '<thead><tr><th style="display: none;">Position</th><th></th><th>Name</th><th>Description</th></tr></thead><tbody>';
            $cnt = 1;
            foreach ($sections as $section) {
                ...
                if (!inArray($section->getName(), Config::surveyCoreSections())) {
    ...
    function showSurveys($message = "") {
        ...
        $surveys = new Surveys();
        $returnStr .= $this->showSurveyList($surveys->getSurveys());
        $returnStr .= '</p></div>    </div>'; //container and wrap
        $returnStr .= $this->showBottomBar();
    ...
    function showSurveyList($surveys) {
        ...
        $returnStr .= '<br/><a href="' . setSessionParams(array('page' => 'sysadmin.survey.addsurvey')) . '">' . 'add new survey' . '</a>';
...
//echo "----";
        ...
        if ($_SESSION['VRFILTERMODE_SURVEY'] == 0) {
            ...
            $returnStr .= '<a href="' . setSessionParams(array('page' => 'sysadmin.survey.addsection')) . '">' . 'add new section' . '</a>';
        } else if ($_SESSION['VRFILTERMODE_SURVEY'] == 1) {
...
Q1
    ...
    function showAnswerTypes($current, $type = -1) {
        ...
        $returnStr .= '$( document ).ready(function() {
                                                $("#answertype").change(function (e) {
                                                    if (this.value == 2 || this.value == 3 || this.value == 15 || this.value == 16) {
                                                        $("#categories").show();
                                                        $("#customanswer").hide();
                                                        $("#section").hide();
                                                    }   
    ...
    function displaySurveySideBarFilter($survey, $filter = 0) {
        ...
        if (inArray($page, array("sysadmin.survey.section")) && $_SESSION['VRFILTERMODE_SECTION'] == 1) {
            ...
            /*
             * 
             * $(document).bind('decidingcontinued.dirtyforms', function() { 
    ...
    function showEditSurvey($suid, $message = "") {
        ...
        $returnStr .= '<tr><td align=top>' . Language::editSurveyName() . '</td><td><textarea cols="60" rows="3" class="form-control" name="' . SETTING_DESCRIPTION . '">' . $survey->getDescription() . '</textarea></td></tr>';
    ...
    function showVariables($variables) {
        ...
        if (sizeof($variables) > 0) {
            ...
            $returnStr .= '<thead><tr><th style="display: none;">Position</th><th></th><th>' . Language::labelTypeEditGeneralName() . '</th><th>' . Language::labelTypeEditGeneralQuestion() . '</th><th>' . Language::labelTypeEditGeneralDescription() . '</th><th>' . Language::labelTypeEditGeneralAnswerType() . '</th></tr></thead><tbody>';
            $cnt = 1;
            foreach ($variables as $variable) {
                $returnStr .= '<tr id="' . $variable->getVsid() . '"><td style="display: none;">' . $cnt . '</td><td>';
                $cnt++;
                //$returnStr .= '<tr><td>';
                ...
                if (!inArray($variable->getName(), Config::surveyCoreVariables())) {
        ...
        $returnStr .= '<a href="' . setSessionParams(array('page' => 'sysadmin.survey.addvariable')) . '">' . 'add new variable' . '</a>';
        $returnStr .= '<div style="min-height: 100px; max-height: 100%;"></div>';
        return $returnStr;
    }
...
$( ".uscic-form-control-admin" ).contextMenu({
    ...
    function showEditVariableGeneral($var) {
        ...
        $returnStr .= '<table width=100%>';
        if (!inArray($var->getName(), Config::surveyCoreVariables())) {
    ...
    function showEditVariableLayout($variable) {
        ...
        if ($answertype == ANSWER_TYPE_TIME) {
            ...
            $returnStr .= "<td>" . $this->displayTooltip(SETTING_SLIDER_TOOLTIP, $variable->getTooltip(), true, $variable->getTyd()) . "</td></tr>";
            $returnStr .= '<tr><td>' . Language::labelTypeEditLayoutTextBox() . '</td>';
            $returnStr .= "<td>" . $this->displayTextBox(SETTING_SLIDER_TEXTBOX, $variable->getTextBox(), true, $variable->getTyd()) . "</td><td width=25><nobr/></td>
                        <td>" . Language::labelTypeEditLayoutStep() . '</td><td>' . $helpstart . '<input type="text" class="form-control autocompletebasic" name="' . SETTING_SLIDER_INCREMENT . '" value="' . $this->displayTextSettingValue(convertHTLMEntities($variable->getIncrement(), ENT_QUOTES)) . '">' . $helpend . '</td></tr>';
            ...
            $s = "";
            if ($variable->getEnumeratedDisplay() == ORIENTATION_CUSTOM) {
            ...
            if ($variable->getEnumeratedDisplay() == ORIENTATION_HORIZONTAL) {
...
//        $returnStr .= '<span class="label label-default">' . Language::labelTypeEditValidationResponse() . '</span>';
        ...
        if (inArray($answertype, array(ANSWER_TYPE_CUSTOM, ANSWER_TYPE_MULTIDROPDOWN, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_SETOFENUMERATED, ANSWER_TYPE_OPEN, ANSWER_TYPE_STRING, ANSWER_TYPE_RANGE, ANSWER_TYPE_SLIDER, ANSWER_TYPE_CALENDAR))) {
        ...
        if (inArray($answertype, array(ANSWER_TYPE_CUSTOM, ANSWER_TYPE_RANGE, ANSWER_TYPE_DOUBLE, ANSWER_TYPE_INTEGER, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_DROPDOWN, ANSWER_TYPE_SETOFENUMERATED, ANSWER_TYPE_MULTIDROPDOWN, ANSWER_TYPE_DATE, ANSWER_TYPE_DATETIME, ANSWER_TYPE_TIME, ANSWER_TYPE_SLIDER))) {
        ...
        if (inArray($answertype, array(ANSWER_TYPE_CUSTOM, ANSWER_TYPE_STRING, ANSWER_TYPE_RANGE, ANSWER_TYPE_DOUBLE, ANSWER_TYPE_INTEGER, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_SETOFENUMERATED, ANSWER_TYPE_DATE, ANSWER_TYPE_DATETIME, ANSWER_TYPE_TIME))) {
            ...
            if (inArray($answertype, array(ANSWER_TYPE_CUSTOM, ANSWER_TYPE_STRING, ANSWER_TYPE_RANGE, ANSWER_TYPE_DOUBLE, ANSWER_TYPE_INTEGER))) {
                ...
                $returnStr .= "<td>" . $helpstart . "<input name='" . SETTING_INPUT_MASK_PLACEHOLDER . "' type='text' class='form-control autocompletebasic' value='" . $this->displayTextSettingValue($variable->getInputMaskPlaceholder()) . "'>" . $helpend . "</td></tr>";
                $returnStr .= '<script type="text/javascript">
                                $( document ).ready(function() {
    ...
    function showEditVariableAssistance($variable) {
        ...
        if (inArray($answertype, array(ANSWER_TYPE_RANGE, ANSWER_TYPE_DOUBLE, ANSWER_TYPE_INTEGER, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_DROPDOWN, ANSWER_TYPE_DATE, ANSWER_TYPE_DATETIME, ANSWER_TYPE_TIME, ANSWER_TYPE_SLIDER))) {
...
//$returnStr .= '<span class="label label-default">' . Language::labelTypeEditOutput() . '</span>';
    ...
    function showTypes($types) {
        ...
        if (sizeof($types) > 0) {
            ...
            $returnStr .= '<thead><tr><th></th><th>Name</th><th>Categories</th></tr></thead><tbody>';
        ...
        $returnStr .= '<a href="' . setSessionParams(array('page' => 'sysadmin.survey.addtype')) . '">' . 'add new type' . '</a>';
        $returnStr .= '<div style="min-height: 100px; max-height: 100%;"></div>';
        return $returnStr;
    }
    ...
    function showEditTypeLayout($type) {
        ...
        if ($answertype == ANSWER_TYPE_TIME) {
            ...
            $returnStr .= "<table>";
            $returnStr .= "<tr><td>" . Language::labelTypeEditLayoutOrientation() . "</td>";
            $returnStr .= "<td>" . $this->displayOrientation(SETTING_SLIDER_ORIENTATION, $type->getSliderOrientation()) . "</td></tr>";
            $returnStr .= "<tr><td>" . Language::labelTypeEditLayoutTooltip() . "</td>";
            $returnStr .= "<td>" . $this->displayTooltip(SETTING_SLIDER_TOOLTIP, $type->getTooltip()) . "</td></tr>";
            $returnStr .= '<tr><td>' . Language::labelTypeEditLayoutTextBox() . '</td>';
            $returnStr .= "<td>" . $this->displayTextBox(SETTING_SLIDER_TEXTBOX, $type->getTextBox(), true) . "</td><td width=25><nobr/></td>
                        <td>" . Language::labelTypeEditLayoutStep() . '</td><td>' . $helpstart . '<input type="text" class="form-control autocompletebasic" name="' . SETTING_SLIDER_INCREMENT . '" value="' . $this->displayTextSettingValue(convertHTLMEntities($type->getIncrement(), ENT_QUOTES)) . '">' . $helpend . '</td></tr>';
            $returnStr .= '<tr><td>' . Language::labelTypeEditLayoutSliderLabelPlacement() . '</td>';
    ...
    function showEditTypeVerification($type) {
        ...
        $t = $type->getAnswerType();
        if (inArray($t, array(ANSWER_TYPE_CUSTOM, ANSWER_TYPE_MULTIDROPDOWN, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_SETOFENUMERATED, ANSWER_TYPE_OPEN, ANSWER_TYPE_STRING, ANSWER_TYPE_RANGE, ANSWER_TYPE_SLIDER, ANSWER_TYPE_CALENDAR))) {
            $returnStr .= '<span class="label label-default">' . Language::labelTypeEditValidationCriteria() . '</span>';
            $returnStr .= "<div class='well'>";
            $returnStr .= "<table>";
            switch ($t) {
        ...
        $helpend2 = '<span class="input-group-addon"><i>' . $message2 . '</i></span></div>';
        if (inArray($t, array(ANSWER_TYPE_CUSTOM, ANSWER_TYPE_RANGE, ANSWER_TYPE_DOUBLE, ANSWER_TYPE_INTEGER, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_DROPDOWN, ANSWER_TYPE_SETOFENUMERATED, ANSWER_TYPE_MULTIDROPDOWN, ANSWER_TYPE_DATE, ANSWER_TYPE_DATETIME, ANSWER_TYPE_TIME, ANSWER_TYPE_SLIDER))) {
            $returnStr .= '<span class="label label-default">' . Language::labelTypeEditValidationComparison() . '</span>';
            $returnStr .= "<div class='well'>";
            $returnStr .= "<table>";
            $returnStr .= "<tr><td>" . Language::labelTypeEditComparisonEqualTo() . "</td>";
    ...
    function showEditTypeAssistance($type) {
        ...
        /* numeric comparison */
        if (inArray($at, array(ANSWER_TYPE_RANGE, ANSWER_TYPE_DOUBLE, ANSWER_TYPE_INTEGER, ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_DROPDOWN, ANSWER_TYPE_DATE, ANSWER_TYPE_DATETIME, ANSWER_TYPE_TIME, ANSWER_TYPE_SLIDER))) {
    ...
    function showGroups($groups) {
        ...
        if (sizeof($groups) > 0) {
            ...
            $returnStr .= '<thead><tr><th></th><th>Name</th><th>Template</th></tr></thead><tbody>';
            $arr = Config::surveyTemplates();
        ...
        $returnStr .= '<a href="' . setSessionParams(array('page' => 'sysadmin.survey.addgroup')) . '">' . 'add new group' . '</a>';
        $returnStr .= '<div style="min-height: 100px; max-height: 100%;"></div>';
        return $returnStr;
    }
    ...
    function showEditGroupGeneral($group) {
        ...
        if ($opendir) {
            ...
            $templates = Config::surveyTemplates();
    ...
    function showEditGroupLayout($group) {
        ...
        if (inArray($group->getTemplate(), array_keys(Config::surveyTableTemplates()))) {
            ...
            if (in_array($group->getTemplate(), array_keys(Config::surveyTableMultiColumnTables()))) {
                ...
                $returnStr .= '<td colspan=4>' . $helpstart . '<input placeholder="' . Language::labelIfEmptyDefaultOrderPlaceholder() . '" style="width: 400px;" type="text" class="form-control autocompletebasic" name="' . SETTING_TABLE_HEADERS . '" value="' . $this->displayTextSettingValue(convertHTLMEntities($group->getTableHeaders(), ENT_QUOTES)) . '">' . $helpend1 . '</td>';
                $returnStr .= "</tr>";
            }
    ...
    function showEditSettingsLayout($message = "") {
        ...
        $templatenames = Config::surveyOverallTemplates();
        $custom = 1;
...
//                $returnStr1 .= '<td>' . $v->getDescription() . '</td>';
        ...
        if ($returnStr1 == "") {
            ...
            $returnStr .= "<input class='btn btn-default' type=button onclick='clearCookie(\"uscicvariablecookie\");' value='" . Language::buttonClear() . "'/>";
    ...
    function showToolsBatchEditorTypes($variablecookievalue, $sectioncookievalue, $groupcookievalue, $typecookievalue) {
        ...
        if ($returnStr1 == "") {
            ...
            $returnStr .= "<input class='btn btn-default' type=button onclick='clearCookie(\"uscictypecookie\");' value='" . Language::buttonClear() . "'/>";
    ...
    function showToolsBatchEditorGroups($variablecookievalue, $sectioncookievalue, $groupcookievalue, $typecookievalue) {
        ...
        if ($returnStr1 == "") {
            ...
            $returnStr .= "<input class='btn btn-default' type=button onclick='clearCookie(\"uscicgroupcookie\");' value='" . Language::buttonClear() . "'/>";
