...
class DisplayQuestion extends DisplayQuestionBasic {
     ...
     function showLanguage() {
        ...
        $variablenames = $this->getRealVariables($this->engine->getDisplayed());
        global $survey;
        //echo $survey->getAllowedLanguages(getSurveyMode());
    ...
    //              $check = ' <span class="glyphicon glyphicon-ok"></span>';
                  ...
                  $returnStr .= '<button type="button" class="btn btn-sm btn-warning" onclick=\'document.getElementById("r").value="' . setSessionsParamString(array_merge(array(SESSION_PARAM_LASTACTION => $this->engine->getLastSurveyAction(), SESSION_PARAM_SURVEY => $survey->getSuid(), SESSION_PARAM_PRIMKEY => $this->primkey, SESSION_PARAM_RGID => $rgid, SESSION_PARAM_VARIABLES => $variablenames, SESSION_PARAM_MODE => getSurveyMode(), SESSION_PARAM_VERSION => getSurveyVersion(), SESSION_PARAM_LANGUAGE => getSurveyLanguage(), SESSION_PARAM_TIMESTAMP => time(), SESSION_PARAM_SEID => $this->engine->getSeid(), SESSION_PARAM_MAINSEID => $this->engine->getMainSeid()), array(SESSION_PARAM_NEWLANGUAGE => $lang["value"]))) . '"; document.getElementById("navigation").value="' . NAVIGATION_LANGUAGE_CHANGE . '"; document.getElementById("form").submit(); \'>' . $lang["name"] . $check . '</button>';
                } 
            }
        }
        return $returnStr;
