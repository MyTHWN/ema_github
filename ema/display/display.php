...
';
        ...
        if ($placement == ERROR_PLACEMENT_WITH_QUESTION) {
            if ($paradata == true) {
                ...
                $errorplacement .= '$.each(errorList, function (index, error) {
					lookupCode($(error.element).attr("name"), $(error.element).attr("name") + "-" + error.message);
                                    });';
            ...
            $errorplacement1 = $errorplacement;
    ...
    $(\'form\').validate({  ' . $rulestringempty . ' 
        ignore: ".dkrfna, :hidden:not(.selectpicker, .bootstrapslider, #calendardiv)", // for selectpicker bootstrap plugin and bootstrap-slider plugin
            onkeyup: false,
            onclick: false,
            onfocusout: false,
    ...
    $(\'form\').validate({  ' . $rulestringerror . ' 
        ignore: ":hidden:not(.selectpicker, .bootstrapslider), #calendardiv, .empty-error", // for selectpicker bootstrap plugin; we dont ignore dkrfna here so we check dk/rf/naed answers
            onkeyup: false,
            onclick: false,
            onfocusout: false,
            highlight: function(element) {
...
$width='330px';
        ...
        if ($orientation == "horizontal") {
            ...
            $returnStr .= '<div class="uscic-' . $orientation . '-slider' . $inlineclass . ' ' . $qa . '">
                                <label>
                                <b>' . $minimumstr . '</b><input id="' . $id . '" ' . $errors . ' class="bootstrapslider ' . $dkrfnaclass . '" style="width: ' . $width . '; height: ' . $height . ';" type="text" name=' . $name . ' value="' . addslashes($value) . '" data-slider-min="' . $minimum . '" data-slider-max="' . $maximum . '" data-slider-step="' . $step . '" data-slider-value="' . addslashes($value) . '" data-slider-orientation="' . $orientation . '" data-slider-selection="after" data-slider-tooltip="' . $tooltip . '"><b>' . $maximumstr . '</b><br/>
                                    </label>';
...
<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
    ...
    function displayInputMasks($name, $current = "") {
        ...
        $selected = array();
        $inputmasks = Config::surveyInputMasks();
...
//          <form class="navbar-form navbar-left" role="search">
    ...
    function displaySelectFromArray($inputArray, $inputSel, $name = 'arrayinput') {
        ...
        $returnStr = '<select class="form-control" name="' . $name . '" style="width:300px" >';
