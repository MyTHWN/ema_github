...
function loadEngine($suid, $primkey, $phpid, $version, $seid, $doState = true, $doContext = true) {
    ...
    if ($row = $db->getRow($r)) {
        ...
        ob_start();
        eval(unserialize(gzuncompress($row["engine"])));
...
function isSurveyMode($l) {
    ...
    // check in modes
    return inArray($l, array_keys(Config::surveyModes()));
}
    ...
    if ($_SESSION['SYSTEM_ENTRY'] == USCIC_SMS) {
        ...
        switch ($utype) {
            case USER_SYSADMIN:
                ...
                return $default;
            case USER_TRANSLATOR:
                $languages = explode("~", $user->getLanguages(getSurveyMode()));
                $default = getDefaultSurveyLanguage();
...
function getFromSessionParams($param) {
    if (loadvar('r') == '') {
