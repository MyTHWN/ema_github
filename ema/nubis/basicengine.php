...
class BasicEngine extends Object {
            ...
            foreach ($fields as $field) {
                ...
                if ($temp != "") {
                    ...
                    if ($cnt != "") {
                        ...
                        if (trim($enumid) != "") {
                            $replacetext = $this->display->showAnswer($cnt, $realfield, $variable, $previousdata, true, $enumid . "_" . $enumvalue);
                        } else {
    ...
    function updateRemarkStatus($dirty = DATA_DIRTY) {
        global $db;
        $query = "update " . Config::dbSurveyData() . "_observations set dirty=$dirty where suid=" . $this->getSuid() . " and primkey='" . $this->getPrimaryKey() . "' and stateid=" . $this->getStateID(); // . " and displayed='" . $this->getDisplayed() . "'";
        //echo $query;
    ...
    function loadRemark() {
        ...
        $query = "select " . $extra . " from " . Config::dbSurveyData() . "_observations where suid=" . $this->getSuid() . " and primkey='" . $this->getPrimaryKey() . "' and stateid=" . $stateid; // . " and displayed='" . $this->getDisplayed() . "'";
...
//echo $current . '---' . $total;
    ...
    function prefixVariableName($variablename) {
        ...
        //echo $vardesc->getSeid() . '||||' .  $this->getSeid();
...
FROM ' . Config::dbSurveyData() . '_states where suid=' . $this->getSuid() . ' and primkey = "' . $this->primkey . '" and mainseid=' . $this->getMainSeid());
    ...
    function doForLoop($min, $max, $counterfield, $loopactions, $looprgid, $nextrgid, $outerloopcounters, $outerlooprgids, $normalfor = 1, $exitfor = 1) {
        ...
        if ($current < $min || $this->isFirstTimeLoop($looprgid, $outerlooprgids)) {
            //$this->firsttimelockloopset[$looprgid] = true;
            //echo '<hr>Set loop: ' . $looprgid . '---' . $current . '---' . $min;
            // store loop data
            global $db;
            $query = "replace into " . Config::dbSurveyData() . "_loopdata (suid, primkey, mainseid, seid, looprgid, loopmin, loopmax, loopcounter,looptype) values (";
            $query .= $this->getSuid() . ", '" . $this->getPrimaryKey() . "', " . $this->getMainSeid() . "," . $this->getSeid() . ", " . $looprgid . "," . $min . "," . $max . ",'" . $counterfield . "'," . $normalfor . ")";
            //echo $query;
            $db->executeQuery($query);
    ...
    function addTimings($lastrgid) {
        ...
        $time = time();
        foreach ($vars as $var) {
            $query = "insert into " . Config::dbSurveyData() . '_times (suid, primkey, rgid, variable, begintime, endtime, timespent, language, mode, version) values (';
    ...
    function logAction($lastrgid, $actiontype) {
        global $logActions;
        $logActions->addAction($this->primkey, '', $lastrgid, USCIC_SURVEY, $actiontype);
    }
        ...
        if ($array != null && $this->forward == true) { // only from memory if going forward, otherwise redo to ensure we get latest fills
            switch ($texttype) {
                case SETTING_OPTIONS:
                    ...
                    $id = "";
                    if (inArray($vardescriptive->getAnswerType(), array(ANSWER_TYPE_ENUMERATED, ANSWER_TYPE_SETOFENUMERATED))) {
