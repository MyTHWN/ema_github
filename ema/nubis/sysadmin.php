...
//                case 'sysadmin.sms.update.res': return $this->showMetaDataUpdateRes(); break;
                ...
                case "sysadmin.users": return $this->showUsers();
                    break;
                case "sysadmin.users.adduser": return $this->showEditUser();
                    break;
    ...
    function showMetaDataSQLUpdateRes() {
        ...
        if (sizeof($tables) > 0 || $updateSql != '') {
            ...
            $message = $displaySms->displaySuccess('Metadata update ready for interviewers.');
        } else {
            $message = $displaySms->displayInfo('Select items to update first.');
        }
    ...
    function showScriptUpdateRes() {
        ...
        if (sizeof($files) > 0) {
            ...
            $message = $displaySms->displaySuccess('Script update ready for interviewers.');
        } else {
            $message = $displaySms->displayInfo('Put some files in /var/haalsi/script first.');
        }
        return $this->showMetaDataUpdate($message);
    }
    ...
    function showEditSurveyRes() {
        ...
        if ($suid != '') { //edit
            ...
            if (loadvar('name') != "") {
                ...
                $var->setQuestion('platform and browser information');
                $var->setTyd(-1);
                $var->setPosition(7);
                $var->save();
...
//ADD ALL SORTS OF CHECKS!!
    ...
    function showEditVariable($vsid = "") {
        $displaySysAdmin = new DisplaySysAdmin();
        if (getFromSessionParams('vsid') != "") {
            $_SESSION['VSID'] = getFromSessionParams('vsid');
        } else if ($vsid != "") {
        ...
        return $displaySysAdmin->showEditVariable($_SESSION['VSID']);
    }
    ...
    function showMoveVariableRes() {
        ...
        if ($vsid != '') { //move
            ...
            if ($suid != $_SESSION['SUID'] || $_SESSION['SEID'] != loadvar("section")) {
                ...
                /* moved section, then recompile variable itself (for section header/footer) */
                else if ($_SESSION['SEID'] != loadvar("section")) {
                    $compiler = new Compiler($_SESSION['SUID'], getSurveyVersion($survey));
                    $mess = $compiler->generateVariableDescriptives(array($variable));
                }
    ...
    function showEditUser() {
        $displayUsers = new DisplayUsers();
        return $displayUsers->showEditUser(getFromSessionParams('urid'));
    }
        ...
        if ($urid != '' && loadvar('name') != "" && loadvar('username') != '') {
            ...
            foreach ($allmodes as $k => $all) {
                if (loadvar(SETTING_USER_MODE . $k) == USER_MODE_YES) {
                    ...
                    if (sizeof($ans) > 0) {
                        $user->setLanguages($k, implode("~", $ans));
                    }
                } else {
                    $user->removeMode($k);
                }
